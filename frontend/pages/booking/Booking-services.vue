<template>
  <div id="screen">
    <section
      ...
      style="height: 40vh; background-image: linear-gradient(rgba(0,0,0,0.4), rgba(0,0,0,0.4)), url('/header/header_camp.webp');"
    ></section>
  </div>
  <div class="">
    <div class="p-0 absolute top-0 w-full z-10">
      ...
      >
        <p class="title text-white lg:text-9xl relative z-10">Book Services</p>
      </div>
    </div>
  </div>
  <section class="content-cabins">
    ...
    <nav class="bg-bg2 px-4 lg:px-10 pt-4 pb-2 text-sm text-gray-500">
      ...
      <NuxtLink to="/Booking" class="hover:text-primary">Booking</NuxtLink>
      <span class="mx-2">/</span>
      <span class="text-black font-semibold">Services</span>
    </nav>
...
useSeo({
  ...
  description:
    "Book extra services at Camp Alta Kiruna including shuttle transfers, sauna sessions, and other add-ons to enhance your Arctic Lapland experience.",
  path: "/booking/Booking-services",
});

const { loading, error, widgetContainer } = useBookingWidget({
  provider: "checkfront",
  trackingTag: "booking_services",
  widgetOptions: {
    category_id: "4",
